...
export class ConditionalNodeTransformer extends Transformer<Pug.ConditionalNode, t.Node> {
    transform(): void {
        try {
            this.output = t.conditionalExpression(parseExpression(this.input.test), 
                this.delegateTo(NodeChildTransformer, this.input.consequent) || t.identifier("undefined"),
                (this.input.alternate && this.delegateTo(NodeChildTransformer, this.input.alternate)) || t.identifier("undefined")
            );
        } catch (e) {
            this.pushError({
